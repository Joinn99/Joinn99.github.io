...
const App: React.FC = () => {
  ...
  // This function is stable and can be defined outside useEffect
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
        if (import.meta.env?.PROD) {
            // In production, use pre-built data from the build step
            try {
                // The data-build.ts file is generated by the `npm run build` script
                const { prebuiltData } = await import('./data-build.ts');
                setData(processData(prebuiltData as any, language));
            } catch (err: any) {
                console.error("Could not load pre-built app data:", err);
                setError("Failed to load content. Please try refreshing the page.");
            }
        } else {
            // In development, fetch YAML files for live updates
            fetchDataDev();
        }
    };
